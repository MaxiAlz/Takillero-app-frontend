...
const DashboardSidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  // Guardar estado expandido desktop
  useEffect(() => {
  ...
  return (
    <>
      ...
      <aside
        ...
        className={`
          flex h-screen flex-col overflow-y-auto overflow-x-hidden bg-black dark:bg-boxdark
          z-50 transform duration-300 ease-in-out
          ...
          ${sidebarExpanded ? 'lg:w-65' : 'lg:w-20'}
        `}
      >
        {/* HEADER */}
        <div className="flex items-center justify-center lg:justify-start px-4 py-5">
          <GiTicket className="text-primary text-2xl" />
          <h1
            className={`ml-2 text-xl font-bold text-primary transition-opacity duration-300
            ${sidebarExpanded || sidebarOpen ? 'opacity-100' : 'opacity-0'}`}
          >
            {APP_TEXT.app_name}
          </h1>
        </div>
        ...
        <nav className="px-2 mt-4 flex-1">
          <ul className="flex flex-col gap-1.5">
            {dashboardItems.map((item) => {
              const isActive =
                pathname === item.link ||
                (item.link !== '/panel' && pathname.startsWith(item.link));
                    ...
                    <SidebarLinkGroup activeCondition={isActive}>
                      {(handleClick, open) => (
                        <>
                          ...
                          {/* SUBITEMS */}
                          {(sidebarExpanded || sidebarOpen) && open && (
...
export const ItemsTemplateRoute = (
  pathname: string,
